...
def retry_with_backoff(
    ...
    raise RuntimeError("retry_with_backoff failed without a last exception")
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for row in results:
        lines.append(
            " | ".join(
                [
                    truncate(row.get("prompt", "")),
                    truncate(row.get("gpt4o_response", "")),
                    truncate(row.get("mini_response", "")),
                    str(row.get("gpt4o_latency", "")),
                    str(row.get("mini_latency", "")),
                ]
            )
        )
