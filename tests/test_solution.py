...
def _load(path: Path, unique_name: str):
    if "." in unique_name:
        package_name = unique_name.rsplit(".", 1)[0]
...
safe_name = DAY_DIR.name.replace("-", "_")

if (SOLUTION_DIR / "solution.py").exists():
    _m = _load(SOLUTION_DIR / "solution.py", f"{safe_name}.solution")
elif (SOLUTION_DIR / "app.py").exists():
    _m = _load(SOLUTION_DIR / "app.py", f"{safe_name}.solution")
else:
    src = "template.py" if (DAY_DIR / "template.py").exists() else "app.py"
    _m = _load(DAY_DIR / src, f"{safe_name}.template")
